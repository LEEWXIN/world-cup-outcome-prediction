...
def add_form_and_h2h(df):
    ...
    Returns df + a 'latest form per team' table used by the dashboard."""
    rows = []
    for _, r in df.iterrows():
...
def train_pipeline(csv_path="international_matches1.csv"):
    ...
    df, elo = add_elo(df)
    df, latest_form = add_form_and_h2h(df)
    ...
    return model, elo, latest_form, metrics
...
def predict_match(model, elo, latest_form, home, away, home_advantage=False):
    ...
    feat = {
        ...
        "Form_GA_Diff": hga - aga, "H2H_Diff": 0.0,
...
if __name__ == "__main__":
    # quick self-test
    m, elo, lf, metrics = train_pipeline("international_matches1.csv")
    ...
    for h, a in [("Brazil", "Qatar"), ("Argentina", "France"), ("Germany", "Japan")]:
        r, lbl, p, f = predict_match(m, elo, lf, h, a, home_advantage=False)
        print(f"\n{h} vs {a}: -> {r}")
        print("   probs:", {k: round(v, 2) for k, v in p.items()})
        print("   Elo diff:", round(f["Elo_Diff"], 1))
